...
public class ReceiptPersist implements PersistFactory {
    ...
    public Receipt add(Object receipt) throws SQLException {
        try {
            Receipt newReceipt = receiptDao.createIfNotExists((Receipt) receipt);
            receiptDao.assignEmptyForeignCollection(newReceipt, "paymentCollection");
            receiptDao.assignEmptyForeignCollection(newReceipt, "taxLineCollection");
            return newReceipt;
        } finally {
            if (connectionSource != null) {
                connectionSource.close();
